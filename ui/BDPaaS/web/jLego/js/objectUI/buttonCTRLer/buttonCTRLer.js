...
jLego.objectUI.buttonCTRLer.prototype.addFreeSingleButton=function(target, option, callbackObject){
    ...
    else if(option.type=='transparent') buttonClass = this.myClass.SINGLE_FREEBUTTON_FRAME_NORMAL;
    ...
    if(option.type=='transparent') $(button).css('color', 'white');
    if(option.style=='small') $(button).addClass(this.myClass.SINGLE_FREEBUTTON_FRAME_SMALL);
    if(option.icon!=null || option.iconURL != null){
        if(option.icon!=null) var iconURL = jLego.func.getImgPath({category: 'buttonIcon', name: option.icon, type: 'png'});
        else if(option.iconURL!=null) var iconURL = option.iconURL;
        var icon = 
                jLego.basicUI.addImg(button, {id: jLego.func.getRandomString(), class: this.myClass.SINGLE_FREEBUTTON_ICON, src: iconURL});
...
jLego.objectUI.buttonCTRLer.prototype.updateFreeSingleButtonStyle=function(button, option){
    //check option
    if(button==null) return null;
    if(option==null)   return button;
    var buttonClass;
    if(option.type=='positive') buttonClass = this.myClass.SINGLE_FREEBUTTON_FRAME + " buttonCTRLer_positiveColor";
    else if(option.type=='info') buttonClass = this.myClass.SINGLE_FREEBUTTON_FRAME + " buttonCTRLer_infoColor";
    else if(option.type=='negtive') buttonClass = this.myClass.SINGLE_FREEBUTTON_FRAME + " buttonCTRLer_negtiveColor";
    ...
    if(option.title){
        $(button).text(option.title);
        $(button).data('icon', null);
    }
    
    if(!option.float){
        if(option.top!=null) $(button).css('top', option.top + 'px');
        if(option.right!=null) $(button).css('right', option.right + 'px');
        else if(option.left!=null) $(button).css('left', option.left + 'px');
    }
    else{
        $(button).css('position', 'relative');
        $(button).css('float', option.float);
        if(option.top!=null) $(button).css('margin-top', option.top + 'px');
        if(option.right!=null) $(button).css('margin-right', option.right + 'px');
        else if(option.left!=null) $(button).css('margin-left', option.left + 'px');
    }
    if(option.style=='small') $(button).addClass(this.myClass.SINGLE_FREEBUTTON_FRAME_SMALL);
    if(option.icon!=null){
        ...
        if(icon == null){
            ...
            $(icon).attr('src', jLego.func.getImgPath({category: 'buttonIcon', name: option.icon, type: 'png'}));
        }
        $(button).css('text-indent', "20px");
    }
...
jLego.objectUI.buttonCTRLer.prototype.addFreeIconButton=function(target, option, callbackObject){
    ...
    if(option.iconURL != null) iconSrc = option.iconURL;
    var button = 
