...
jLego.objectUI.statusMonitor.prototype.addIconMonitor=function(target, option){
    ...
    if(option.initStatus=="ok"){
        ...
        if(option.theme == "white") statusImage = jLego.func.getImgPath({category: 'statusLarge', name: 'okWhite', type: 'png'});
        status = 'ok';
        ...
        if(option.theme == "white") statusImage = jLego.func.getImgPath({category: 'statusLarge', name: 'degradedWhite', type: 'png'});
        status = 'degraded';
    ...
    else if(option.initStatus=="failed" || option.initStatus=="error"){
        statusImage = jLego.func.getImgPath({category: 'statusLarge', name: 'failed', type: 'png'});
        if(option.theme == "white") statusImage = jLego.func.getImgPath({category: 'statusLarge', name: 'failedWhite', type: 'png'});
        status = 'failed';
        ...
        if(option.theme == "white") statusImage = jLego.func.getImgPath({category: 'statusLarge', name: 'unknownWhite', type: 'png'});
        status = 'unknown';
    ...
    if(option.hideTitle == true) $(titleElement).hide();
    ...
    $(mainElement).data('theme', option.theme);
    return mainElement;
}
...
jLego.objectUI.statusMonitor.prototype.updateIconMonitor=function(monitor, targetStatus, targetStatusText){
    ...
    var theme = $(monitor).data('theme');
    if(targetStatus=="ok"){
        statusImage = jLego.func.getImgPath({category: 'statusLarge', name: 'ok', type: 'png'});
        if(theme == "white") statusImage = jLego.func.getImgPath({category: 'statusLarge', name: 'okWhite', type: 'png'});
        status = 'ok';
        statusText = this.myLanguage.OK;
        ...
        statusImage = jLego.func.getImgPath({category: 'statusLarge', name: 'degraded', type: 'png'});
        if(theme == "white") statusImage = jLego.func.getImgPath({category: 'statusLarge', name: 'degradedWhite', type: 'png'});
        status = 'degraded';
        statusText = this.myLanguage.DEGRADED;
        if(targetStatusText!=null) statusText = targetStatusText;
        statusClass = this.myClass.STATUSMONITOR_ICONMONITOR_STATUSTEXT_DEGRADED;
    ...
    else if(targetStatus=="failed" || targetStatus == "error"){
        statusImage = jLego.func.getImgPath({category: 'statusLarge', name: 'failed', type: 'png'});
        if(theme == "white") statusImage = jLego.func.getImgPath({category: 'statusLarge', name: 'failedWhite', type: 'png'});
        status = 'failed';
        statusText = this.myLanguage.FAILED;
        if(targetStatusText!=null) statusText = targetStatusText;
        statusClass = this.myClass.STATUSMONITOR_ICONMONITOR_STATUSTEXT_FAILED;
        ...
        statusImage = jLego.func.getImgPath({category: 'statusLarge', name: 'unknown', type: 'png'});
        if(theme == "white") statusImage = jLego.func.getImgPath({category: 'statusLarge', name: 'unknownWhite', type: 'png'});
        status = 'unknown';
        statusText = this.myLanguage.UNKNOWN;
        if(targetStatusText!=null) statusText = targetStatusText;
        statusClass = this.myClass.STATUSMONITOR_ICONMONITOR_STATUSTEXT_CLOSE;
...
jLego.objectUI.statusMonitor.prototype.addBigRoundMonitor=function(target, option){
    ...
    if(option.hasUnit==false){
        $(unitElement).hide();
        $(valueElement).height($(roundBackgroundElement).height());
        $(valueElement).css('margin-top', '0px');
        $(valueElement).css('line-height', $(roundBackgroundElement).height() + "px");
    }
    var titleElement =
            jLego.basicUI.addDiv(mainElement, {id: defID + "_title", class: this.myClass.STATUSMONITOR_ICONMONITOR_TITLE});
    $(titleElement).text(option.title);
