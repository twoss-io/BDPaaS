...
#
# Manage user namespaces & Apex jobs
#
#

JOBS_PATH="./modules/apex/"
JOBS=(\
	apex-sandbox-deploy-1.yaml)
SPECS=(\
	apex-sandbox)
SVCS=(\
	apex-sandbox-webui)
...
start_apex_job()
{
	...
	_ns=$1
	echo -e "\nStart apex jobs in namespace: "${_ns}"\n"
	...
	# create services
	_pod=`kubectl -n ${_ns} get po |tail -n +2 |awk -F' ' '{print $1}'`
	kubectl -n ${_ns} expose po ${_pod} --type LoadBalancer --name=${SVCS[0]}
...
stop_apex_job()
{
	...
	_ns=$1
	echo -e "\nStop apex jobs in namespace: "${_ns}"\n"
	...
	# delete services
	kubectl -n ${_ns} delete svc ${SVCS[0]}
...
query_apex_url()
{
	...
	_rr=`kubectl -n ${_ns} get svc | grep '^'${SVCS[0]}' '`
	[[ $? != 0 ]] && echo "      [ERR]!" && return 1
	_nn=${#PORTS[*]}
	let "_n = ${_nn} - 1"
	_js="{"
        for(( i=0;i<${_nn};i++ )); do
		if [[ ${_has_type} = "" ]]; then
			_url=`echo "${_rr}" | awk -F' ' '{print $4}' | sed 's/,/\n/g' | grep ${PORTS[$i]} | awk -F'[:/]' '{print $2}'`
		else
			_url=`echo "${_rr}" | awk -F' ' '{print $5}' | sed 's/,/\n/g' | grep ${PORTS[$i]} | awk -F'[:/]' '{print $2}'`
		fi
...
[ -z $1 ] && echo -e "\n[ERR] Pls specify namespace to run apex jobs\n" && exit 1
...
#				echo -e "\n   [ERR]: incorrect interface!\n"
				...
				query_apex_url ${ns} ${k8s_server_eth0}
				ret=$?
			fi
		fi
		;;
